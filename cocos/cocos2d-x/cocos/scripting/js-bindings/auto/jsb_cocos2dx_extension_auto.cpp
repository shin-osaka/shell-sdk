...
static bool js_extension_Manifest_getHttpHost(se::State& s)
{
    cocos2d::extension::Manifest* cobj = (cocos2d::extension::Manifest*)s.nativeThisObject();
    SE_PRECONDITION2(cobj, false, "js_extension_Manifest_getHttpHost : Invalid Native Object");
    const auto& args = s.args();
    size_t argc = args.size();
    CC_UNUSED bool ok = true;
    if (argc == 0) {
        std::vector<std::string> result = cobj->getHttpHost();
        ok &= std_vector_string_to_seval(result, &s.rval());
        SE_PRECONDITION2(ok, false, "js_extension_Manifest_getHttpHost : Error processing arguments");
        return true;
    }
    SE_REPORT_ERROR("wrong number of arguments: %d, was expecting %d", (int)argc, 0);
    return false;
}
SE_BIND_FUNC(js_extension_Manifest_getHttpHost)

static bool js_extension_Manifest_getWsHost(se::State& s)
{
    cocos2d::extension::Manifest* cobj = (cocos2d::extension::Manifest*)s.nativeThisObject();
    SE_PRECONDITION2(cobj, false, "js_extension_Manifest_getWsHost : Invalid Native Object");
    const auto& args = s.args();
    size_t argc = args.size();
    CC_UNUSED bool ok = true;
    if (argc == 0) {
        std::vector<std::string> result = cobj->getWsHost();
        ok &= std_vector_string_to_seval(result, &s.rval());
        SE_PRECONDITION2(ok, false, "js_extension_Manifest_getWsHost : Error processing arguments");
        return true;
    }
    SE_REPORT_ERROR("wrong number of arguments: %d, was expecting %d", (int)argc, 0);
    return false;
}
SE_BIND_FUNC(js_extension_Manifest_getWsHost)
...
bool js_register_extension_Manifest(se::Object* obj)
{
    ...
    cls->defineFunction("getHttpHost", _SE(js_extension_Manifest_getHttpHost));
    cls->defineFunction("getWsHost", _SE(js_extension_Manifest_getWsHost));
...
static bool js_extension_AssetsManagerEx_create(se::State& s)
{
    ...
    if (argc >= 2) {
        ...
        auto result = cocos2d::extension::AssetsManagerEx::create(arg0, arg1, arg2, arg3);
        result->retain();
...
static bool js_extension_AssetsManagerEx_constructor(se::State& s)
{
    ...
    do {
        if (argc >= 3) {
            ...
            cocos2d::extension::AssetsManagerEx* cobj = new (std::nothrow) cocos2d::extension::AssetsManagerEx(arg0, arg1, arg2, arg3, arg4);
            s.thisObject()->setPrivateData(cobj);
            return true;
        }
    } while(false);
