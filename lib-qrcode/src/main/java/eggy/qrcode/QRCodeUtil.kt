...
import eggy.qrcode.zxing.EncodeHintType
import eggy.qrcode.zxing.QRCodeWriter
import eggy.qrcode.zxing.WriterException
 ...
 */
object QRCodeUtil {
    val TAG = QRCodeUtil::class.java.simpleName
     ...
     * @param content          字符串内容
     * @param size             位图宽&高(单位:px)
     * @param characterSet    字符集/字符转码格式 (支持格式:[CharacterSetECI])。传null时,zxing源码默认使用 "ISO-8859-1"
     ...
     * @param tb     目标图片 (如果targetBitmap != null, 黑色色块将会被该图片像素色值替代)
    ...
    fun createQRCodeBitmap(
        ...
        try {
            ...
            val bitMatrix = QRCodeWriter().encode(content, size, size, hints)
     ...
     * @param srcBitmap   原图片
     * @param logoBitmap  logo图片
     * @param percent 百分比 (用于调整logo图片在原图片中的显示大小, 取值范围[0,1], 传值不合法时使用0.2F)
