...
internal object MaskUtil {
    val TAG = MaskUtil::class.java.simpleName
    ...
    private fun isWhiteHorizontal(rowArray: ByteArray, ifrom: Int, ito: Int): Boolean {
        val from = ifrom.coerceAtLeast(0)
        val to = ito.coerceAtMost(rowArray.size)
    ...
    private fun isWhiteVertical(array: Array<ByteArray>, col: Int, ifrom: Int, ito: Int): Boolean {
        val from = ifrom.coerceAtLeast(0)
        val to = ito.coerceAtMost(array.size)
        for (i in from until to) {
    ...
    fun applyMaskPenaltyRule4(matrix: ByteMatrix): Int {
        ...
        val numTotalCells = matrix.getHeight() * matrix.getWidth()
        val fivePercentVariances = Math.abs(numDarkCells * 2 - numTotalCells) * 10 / numTotalCells
    ...
    private fun applyMaskPenaltyRule1Internal(matrix: ByteMatrix, isHorizontal: Boolean): Int {
        var penalty = 0
        val iLimit = if (isHorizontal) matrix.getHeight() else matrix.getWidth()
        val jLimit = if (isHorizontal) matrix.getWidth() else matrix.getHeight()
        val array = matrix.getBytes()
