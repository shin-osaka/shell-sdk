...
import eggy.qrcode.zxing.MaskUtil
...
internal object MatrixUtil {
    val TAG = MatrixUtil::class.java.simpleName
    ...
    fun embedTypeInfo(ecLevel: ErrorCorrectionLevel, maskPattern: Int, matrix: ByteMatrix) {
        ...
        for (i in 0 until typeInfoBits.getSize()) {
            val bit = typeInfoBits[typeInfoBits.getSize() - 1 - i]
            val x1 = TYPE_INFO_COORDINATES[i][0]
            val y1 = TYPE_INFO_COORDINATES[i][1]
            matrix[x1, y1] = bit
            if (i < 8) {
                val x2 = matrix.getWidth() - i - 1
                val y2 = 8
                matrix[x2, y2] = bit
            } else {
                val x2 = 8
                val y2 = matrix.getHeight() - 7 + (i - 8)
                matrix[x2, y2] = bit
            }
        }
    }
    ...
    fun maybeEmbedVersionInfo(version: Version, matrix: ByteMatrix) {
        ...
        for (i in 0..5) {
            for (j in 0..2) {
                ...
                bitIndex--
                matrix[i, matrix.getHeight() - 11 + j] = bit
                matrix[matrix.getHeight() - 11 + j, i] = bit
            }
        }
    }
    ...
    fun embedDataBits(dataBits: BitArray, maskPattern: Int, matrix: ByteMatrix) {
        ...
        if (bitIndex != dataBits.getSize()) {
            throw WriterException("Not all bits consumed: " + bitIndex + '/' + dataBits.getSize())
        }
    }
    ...
    fun calculateBCHCode(v: Int, poly: Int): Int {
        require(poly != 0) { "0 polynomial" }
        val msbSetInPoly = findMSBSet(poly)
        var value = v shl msbSetInPoly - 1
    ...
    fun makeTypeInfoBits(ecLevel: ErrorCorrectionLevel, maskPattern: Int, bits: BitArray) {
        ...
        val typeInfo = ecLevel.getBits() shl 3 or maskPattern
    ...
    fun makeVersionInfoBits(version: Version, bits: BitArray) {
        ...
        val bchCode = calculateBCHCode(version.getVersionNumber(), VERSION_INFO_POLY)
    ...
    private fun embedTimingPatterns(matrix: ByteMatrix) {
        for (i in 8 until matrix.getWidth() - 8) {
            val bit = (i + 1) % 2
    ...
    private fun embedDarkDotAtLeftBottomCorner(matrix: ByteMatrix) {
        if (matrix[8, matrix.getHeight() - 8].toInt() == 0) {
            throw WriterException()
        }
        matrix[8, matrix.getHeight() - 8] = 1
    }
    ...
    private fun embedPositionDetectionPatternsAndSeparators(matrix: ByteMatrix) {
        ...
        embedVerticalSeparationPattern(
            vspSize, matrix.getHeight() - vspSize,
            matrix
        )
    }
    ...
    private fun maybeEmbedPositionAdjustmentPatterns(version: Version, matrix: ByteMatrix) {
        ...
        val index = version.getVersionNumber() - 1
