 ...
 */
internal class GenericGF(private val primitive: Int,private val size: Int, private val generatorBase: Int) {
    private val expTable: IntArray = IntArray(size)
    private val logTable: IntArray = IntArray(size)
    private val zero: GenericGFPoly
    private val one: GenericGFPoly
    ...
    fun getZero(): GenericGFPoly {
        return zero
    }

    fun getOne(): GenericGFPoly {
        return one
    }
    ...
    fun multiply(a: Int, b: Int): Int {
        return if (a == 0 || b == 0) {
            0
        } else expTable.get((logTable.get(a) + logTable.get(b)) % (size - 1))
    }
    ...
    companion object {
        val TAG = GenericGF::class.java.simpleName
