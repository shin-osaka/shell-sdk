 ...
 */
object Encoder {
    val TAG = Encoder::class.java.simpleName
    ...
    fun encode(
        ...
        val numLetters = if (mode == Mode.BYTE) dataBits.getSizeInBytes() else content.length
        appendLengthInfo(numLetters, version, mode, headerAndDataBits)
        headerAndDataBits.appendBitArray(dataBits)
        val ecBlocks = version.getECBlocksForLevel(ecLevel)
        val numDataBytes = version.getTotalCodewords() - ecBlocks.getTotalECCodewords()
    ...
    private fun chooseMode(content: String, encoding: String): Mode {
        ...
        for (element in content) {
            ...
            if (c >= '0' && c <= '9') {
                hasNumeric = true
            } else if (getAlphanumericCode(c.code) != -1) {
    ...
    private fun willFit(
        ...
        val numBytes = version.getTotalCodewords()
        val ecBlocks = version.getECBlocksForLevel(ecLevel)
        val numEcBytes = ecBlocks.getTotalECCodewords()
    ...
    fun terminateBits(numDataBytes: Int, bits: BitArray) {
        ...
        if (bits.getSize() > capacity) {
            throw WriterException(
                "data bits cannot fit in the QR Code" + bits.getSize() + " > " +
                        capacity
            )
        }
        run {
        ...
        val numBitsInLastByte = bits.getSize() and 0x07
        if (numBitsInLastByte > 0) {
            for (i in numBitsInLastByte..7) {
                bits.appendBit(false)
            }
        }
        val numPaddingBytes = numDataBytes - bits.getSizeInBytes()
    ...
    fun interleaveWithECBytes(
        ...
        for (i in 0 until maxNumDataBytes) {
            for (block in blocks) {
                val dataBytes = block.getDataBytes()
                if (i < dataBytes.size) {
        ...
        for (i in 0 until maxNumEcBytes) {
            for (block in blocks) {
                val ecBytes = block.getErrorCorrectionBytes()
                if (i < ecBytes.size) {
